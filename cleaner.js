...
(function () {
  ...
  function isReplyPost(tweetElement) {
    try {
      const replyIndicators = ["回复", "Replying to", "你回复了", "你已回复"];
      const fullText = tweetElement.textContent || "";
  ...
  function isQuotePost(tweetElement) {
    try {
      ...
      const quoteIndicators = ["引用", "Quote", "Quoted post", "引用推文"];
      const fullText = tweetElement.textContent || "";
  ...
  function findTweetElements() {
    ...
    if (currentHandle) {
      ...
      const filtered = tweets.filter(
        (el) => isTweetByCurrentUser(el) || isRetweetedPost(el)
      );
      if (filtered.length !== tweets.length) {
        log(
          `按当前主页用户 @${currentHandle} 过滤（含转推）: ${filtered.length}/${tweets.length}`,
          "debug"
        );
      }
      // 调试：输出简略列表
      if (CONFIG.debug && filtered.length > 0) {
        ...
        try {
          ...
          log(JSON.stringify(brief), "debug");
        }
      }
      return filtered;
    }
  ...
  async function processNextTweet() {
    ...
    const freshElement =
      findTweetElementById(tweetInfoFromBatch.id) || tweetInfoFromBatch.element;
    const tweetInfo = freshElement
      ? { ...tweetInfoFromBatch, element: freshElement }
      : tweetInfoFromBatch;
    ...
    log(
      `📝 内容: ${tweetInfo.text.substring(0, 80)}${
        tweetInfo.text.length > 80 ? "..." : ""
      }`
    );
  ...
  async function cleanupTweets() {
    ...
    while (
      stats.scrollAttempts < CONFIG.maxScrollAttempts &&
      stats.processed < CONFIG.maxTweets
    ) {
      ...
      if (result === null) {
        ...
        if (consecutiveEmptyPages >= CONFIG.emptyPageStopThreshold) {
          log(
            `⏹️ 连续 ${CONFIG.emptyPageStopThreshold} 次页面为空，可能账号无内容，停止`,
            "info"
          );
          break;
        }
  ...
  function showProgress() {
    log("\n=== 📊 进度更新 ===", "info");
    log(
      `已处理: ${stats.processed} | 已删除: ${stats.deleted} | 跳过: ${stats.skipped} | 错误: ${stats.errors}`
    );
  ...
  function showFinalStats() {
    ...
    const successRate =
      stats.processed > 0
        ? ((stats.deleted / stats.processed) * 100).toFixed(1)
        : 0;
