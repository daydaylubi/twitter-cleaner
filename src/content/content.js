...
import { ContentScriptMessageManager } from '../utils/messaging.js';
...
export class TwitterCleaner {
  constructor() {
    ...
    this.messaging = new ContentScriptMessageManager();
  ...
  setupMessageHandlers() {
    // 开始清理
    this.messaging.registerHandler('START_CLEANING', async (payload) => {
      await this.startCleaning(payload);
      return { success: true };
    });

    // 停止清理
    this.messaging.registerHandler('STOP_CLEANING', async (payload) => {
      await this.stopCleaning();
      return { success: true };
    });

    // 获取状态
    this.messaging.registerHandler('GET_STATUS', async (payload) => {
      return {
    ...
    // 重置进度
    this.messaging.registerHandler('RESET_PROGRESS', async (payload) => {
    ...
    while (
      ...
      try {
        ...
        if (batchSize === 0) {
          ...
          this.logger.warning(`页面无推文，尝试滚动加载 (${consecutiveEmptyPages}/${this.config.emptyPageStopThreshold})`);
  ...
  async processTweet(tweetInfo) {
    try {
      ...
      if (!shouldProcess) {
        this.logger.debug(`跳过推文: 日期较新 (${tweetInfo.date.toLocaleDateString()})`);
        this.stats.skipped++;
        return 'skipped';
      }
      ...
      // 执行删除操作
      const success = await this.deleter.deleteTweet(tweetInfo.element, tweetInfo.type);
  ...
  async scrollDown() {
    try {
      ...
      this.logger.debug(`滚动${hasNewContent ? '成功' : '无效'}: ${beforeScrollHeight} → ${afterScrollHeight}`);
  ...
  async sendStatusUpdate(currentTweet = null) {
    try {
      await this.messaging.sendProgressUpdate(this.stats, currentTweet);
    } catch (error) {
